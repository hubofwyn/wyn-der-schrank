...
- __Active:__ G15 PWA Shell and Session Polish [feat/resize-pipeline]
- __Next ready:__ none
- __Blocked:__ none
- __Last milestone:__ 2026-03-10 — G14 Resize Pipeline complete (573 tests)
- __Gates:__ all green (573 tests)
...
- __2026-03-10__ — G14: Resize Pipeline — debounce utility (9 tests), PhaserViewport debounced resize (100ms), main.ts setGameSize wiring, BaseScene subscribeResize/onViewportResize lifecycle, 12 UI scenes subscribe, portrait orientation overlay (560 tests†) [feat/resize-pipeline]
- __2026-03-10__ — Vitest config migration: replaced deprecated `vitest.workspace.ts` with root `vitest.config.ts` (`test.projects`), explicit `--config` in scripts. Fixed test discovery leak (dist test files + bun cache symlinks inflated prior counts by ~18). †Corrected test counts: G14=560, prior entries slightly inflated.
