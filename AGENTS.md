...
docs/
  ...
  plans/
    ...
    studio-asset-interface.md → Shared contract with asset studio repo (@hub-of-wyn/shared publishing)
...
- If an interface is used by two or more domain modules, define it once in a dedicated `modules/<name>/` directory or in `@hub-of-wyn/shared`. Never duplicate interfaces across modules.
...
- Schema changes in `@hub-of-wyn/shared` (contract changes affect client, server, and studio)
...
`@hub-of-wyn/shared` is published to npm public registry. The studio repo depends on it.
...
| Scene layout helpers | `apps/client/src/modules/ui/scene-layout.ts` (safe-zone positioning, `scaledStyle()`, `hitArea()`) |
